...
def _default_scope() -> dict[str, Any]:
    ...
    read_repos: list[str] = []
    if "pivony-api-dev" in ids:
        read_repos.append("pivony-api-dev")
    return {
...
def write_scope(write_repo: str, read_repos: list[str]) -> dict[str, Any]:
    ...
    payload = {
        "masterr_root": scope["masterr_root"],
        "write_repo": write_repo,
        "read_repos": cleaned,
    }
...
def apply_scope_to_env(env: dict[str, str] | None = None) -> dict[str, str]:
    ...
    if read_paths:
        out["QUALITY_LOOP_API_REPO"] = read_paths[0]["path"]
    return out
    ...
    return {
        **scope,
        ...
        "prefix_help": (
            ...
            "Örn: pivony-api-dev/api/routes.py, src/core/prompts.py"
        ),
    }
