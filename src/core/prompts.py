...
- `get_distribution(dashboard_id, kind, pivot_column, ...)`: a breakdown by `kind`, read from the SAME engine as the dashboard widgets (matches the page 1:1) — 'sentiment' (pos/neu/neg/mixed = Sentiment Analysis), 'intent' (why customers write = Intent Analysis), 'platform' (channel mix), or 'pivot' (a Pivot Analysis column like hasChild/channel; pass `pivot_column`, or omit it first to list the available columns). Use for "intent oranı", "duygu dağılımı", "hangi kanaldan", "hasChild/çocuklu oranı", "pivot dağılımı".
