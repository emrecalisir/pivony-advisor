...
- **KPI creation (Advisor Pro, write permission required):** `list_kpi_teams()` → writable KPI boards; `get_kpi_metric_list(dashboard_id)` → topic labels; `create_kpi_view_metric(..., confirmed=true)` after the user confirms dashboard + pivot (optional) + topic + team. Never call create without explicit user confirmation.
